...
[결과 안내 구성 규칙]
Final Answer 바로 앞에 안내 문구를 다음과 같은 포맷으로 구성하세요. 
...
🎁 추천 방향
친구의 **새로운 시작을 응원**하고, **실용적이거나 위트 있는 선물**을 주는 게 좋아요. 감정적으로는 "넌 잘할 수 있어!", "축하해, 늘 응원해"라는 메시지가 느껴져야 하고. 그러면 아래와 같은 컨셉으로 추천할 수 있어요.

---

✨ 포장/전달 팁
* **응원 카드**는 꼭 같이 전달하는 것을 추천해요! “너라면 잘 해낼 거야”, “앞으로의 네가 더 기대돼” 같은 말 한 줄이면 충분해요.
* **포장지나 박스 색깔**은 “차분한 네이비”나 “밝은 민트톤”처럼 깔끔하고 새로운 시작에 어울리는 분위기로 추천해요.

---

혹시 친구가 **어떤 직무나 업종**으로 취업했는지 알 수 있을까요? (예: 디자이너, 마케터, 개발자 등)
그걸 알면 조금 더 **맞춤형 추천**도 가능해요!
...
def call_agent(input_text, agent_executor, state=None):
    ...
    if header_cleaned:
        explanation_lines = header_cleaned.split("\n")
        spaced_text = "\n\n".join([line.strip() for line in explanation_lines if line.strip()])
        result.append({
            "EXPLANATION": spaced_text
        })
