...
- 수령인의 관계, 성별, 연령대 정보는 반드시 Action Input에 명시적으로 포함하세요.
- 수령인이 가족(예: 어머니, 아버지, 여동생 등)일 경우, 반드시 해당 단어를 그대로 사용하세요.  
- "친구", "지인", "상대방" 등의 일반화된 단어로 바꾸지 마세요.
- 관계 정보가 모호하거나 누락되면, 추천 정확도가 떨어지므로 반드시 포함되어야 합니다.
...
[추천 규칙]
- 반드시 수령인의 성별, 연령대, 관계, 감정, 예산, 스타일, 친밀도 등을 모두 고려하세요.- 수령인 정보(관계, 성별, 연령대)는 반드시 경험 문장에 포함되어야 합니다.
